...
public sealed class OcrPreprocessService
{
    ...
    public Bitmap Apply(Bitmap source, AppSettings settings, int? thresholdOverride, bool? autoThresholdOverride)
    {
        if (!settings.EnableOcrBinarization &&
            !settings.EnableOcrGamma &&
            !settings.EnableOcrGrayscale &&
            !settings.EnableOcrContrast)
        {
            return source;
        }
        ...
        void ApplyStageIfEnabled(bool enabled, Func<Bitmap, Bitmap> stage)
        {
            if (!enabled)
            {
                return;
            ...
            var next = stage(current);
            if (ownsCurrent && !ReferenceEquals(current, next))
            {
                current.Dispose();
            }

            current = next;
            ownsCurrent = !ReferenceEquals(current, source);
        }

        if (settings.EnableOcrGamma)
        {
            ApplyStageIfEnabled(true, bitmap => ApplyGammaCorrection(bitmap, settings.OcrGamma));
        }

        if (settings.EnableOcrGrayscale)
        {
            ApplyStageIfEnabled(true, ApplyGrayscale);
        }

        if (settings.EnableOcrContrast)
        {
            ApplyStageIfEnabled(true, bitmap => ApplyContrast(bitmap, settings.OcrContrast));
        }
        ...
        if (ownsCurrent && !ReferenceEquals(current, output))
        {
            current.Dispose();
        }
        ...
        try
        {
            ...
            for (var y = 0; y < height; y++)
            {
                ...
                for (var x = 0; x < width; x++)
                {
                    ...
                    var a = inputBuffer[inputIndex + 3];
                    var luma = (byte)Math.Clamp((int)Math.Round((0.299 * r) + (0.587 * g) + (0.114 * b)), 0, 255);

                    var outputIndex = outputRow + (x * 4);
                    outputBuffer[outputIndex] = luma;
                    outputBuffer[outputIndex + 1] = luma;
                    outputBuffer[outputIndex + 2] = luma;
                    outputBuffer[outputIndex + 3] = a;
                }
            }
        ...
        try
        {
            ...
            for (var y = 0; y < height; y++)
            {
                ...
                for (var x = 0; x < width; x++)
                {
                    ...
                    var outB = (byte)Math.Clamp((int)Math.Round(((b - 128.0) * contrast) + 128.0), 0, 255);
                    var outG = (byte)Math.Clamp((int)Math.Round(((g - 128.0) * contrast) + 128.0), 0, 255);
                    var outR = (byte)Math.Clamp((int)Math.Round(((r - 128.0) * contrast) + 128.0), 0, 255);

                    var outputIndex = outputRow + (x * 4);
                    outputBuffer[outputIndex] = outB;
                    outputBuffer[outputIndex + 1] = outG;
                    outputBuffer[outputIndex + 2] = outR;
                    outputBuffer[outputIndex + 3] = a;
                }
            }
