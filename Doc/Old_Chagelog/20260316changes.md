...
**2026-03-16 10:56 (Asia/Taipei) — Document OBS-referenced hook pipeline performance policy**

### Summary
- OBS の `graphics-hook` 実装を参照し、現行 Hook pipeline の性能改善方針を `Doc/` に整理した。

### Context / Goal
- 現行 Hook pipeline は delayed readback の土台はあるが、`Present` / `vkQueuePresentKHR` hot path に CPU publish が残っており、改善余地があった。
- OBS の Github コードを一次情報として比較し、次に着手すべき改善順序を明確にしたかった。

### Changes
- OBS `graphics-hook` の D3D11 shared memory 経路と copy thread 設計を確認し、ローカル実装との差分を整理した。
- `Doc/GraphicsHook_OBS_Reference_Performance_Improvement_Policy.md` を追加し、短期・中期・長期の改善方針を文書化した。
- 改善対象を「off-Present publish」「consumer 側 reopen / sleep retry 削減」「shared texture bridge は第3段階」に整理した。

### Files Touched
- `Doc/GraphicsHook_OBS_Reference_Performance_Improvement_Policy.md` — OBS 参照コード、現状差分、改善方針、実装ステップ、計測観点を新規整理した。
- `.agent/changes.md` — 本タスクの記録を追記した。

### Behavioral Impact
- 実行時挙動の変更はまだない。
- 次の実装フェーズで優先すべき変更順序と評価指標が明確になった。

### Risk & Mitigation
- Risk: OBS 設計をそのまま移植すると、OCR 前提の CPU consumer 契約と噛み合わず過剰設計になる。
- Mitigation: shared memory 契約維持を前提に、まず off-Present publish と consumer 側の reopen 削減を優先する方針に限定した。

### Tests / Verification
- OBS 公式リポジトリ `obsproject/obs-studio` の commit `5533a277e4400203b50114c993f89878d50a27b9` を参照してコード確認
- ローカル実装の `Dx11PresentHook.cpp` / `VulkanPresentHook.cpp` / `SharedFrameWriter.cpp` / `GraphicsHookCaptureProvider.cs` を比較確認

**2026-03-16 11:11 (Asia/Taipei) — Add DX11 off-Present publish implementation plan**

### Summary
- DX11 の `off-Present publish` に絞った実装案を `Doc/` に追加した。

### Context / Goal
- 既存の DX11 delayed readback 案は `Map` 待ちの削減までを主眼としており、`Present` hot path に残る CPU publish の分離案を個別に詰める必要があった。
- まず DX11 だけで worker publish の責務分離、slot 所有権、teardown 方針を明文化したかった。

### Changes
- DX11 専用の `off-Present publish` 実装案を新規作成した。
- worker thread、publish queue、slot state 拡張、generation ベースの stale request 廃棄方針を整理した。
- `Unmap` の thread ownership が論点になる点をリスクとして明記した。

### Files Touched
- `Doc/GraphicsHook_DX11_OffPresentPublish_Implementation_Plan.md` — DX11 の worker publish 導入案、状態遷移、実装手順、リスクを新規整理した。
- `.agent/changes.md` — 本タスクの記録を追記した。

### Behavioral Impact
- 実行時挙動の変更はまだない。
- DX11 の次実装フェーズで何を切るかが `off-Present publish` に限定して明確になった。

### Risk & Mitigation
- Risk: DX11 の `Unmap` を worker thread 側に寄せる設計は D3D11 immediate context の扱いで再検証が必要になる。
- Mitigation: 文書内で worker `Unmap` 案と render-thread cleanup 案の両方を残し、実装前確認事項として固定した。

### Tests / Verification
- 未実施（ドキュメント追加のみ）

**2026-03-16 11:39 (Asia/Taipei) — Implement DX11 off-Present publish worker path**

### Summary
- DX11 Hook の delayed readback 経路に publish worker を導入し、通常の `Present` hot path から CPU publish を外した。

### Context / Goal
- 既存 DX11 Hook は `Map(DO_NOT_WAIT)` による delayed readback までは入っていたが、`Present` 内で `memcpy(rt.scratch)` と `SharedFrameWriter.WriteFrame(...)` を実行していた。
- `Doc/GraphicsHook_OBS_Reference_Performance_Improvement_Policy.md` と `Doc/GraphicsHook_DX11_OffPresentPublish_Implementation_Plan.md` の方針どおり、CPU publish を worker thread 側へ逃がしたかった。

### Changes
- `Dx11PresentHook.cpp` に publish worker thread、publish queue、completed-slot cleanup を追加した。
- `TryPublishPendingCaptureLocked(...)` を「`Map(DO_NOT_WAIT)` 成功後に worker へ enqueue」する経路へ変更し、通常 delayed readback path から CPU copy と `WriteFrame` を外した。
- `Unmap` は immediate context の扱いを保守的にするため worker ではなく次回 `Present` / reset / uninstall 側 cleanup に残した。
- `frameId` と `lastCaptureQpc` を worker から更新できるよう最小限 atomic 化した。
- install 失敗時、resize、detach で worker を drain / stop する rollback を追加した。

### Files Touched
- `Native/HookAgentDx11/Dx11PresentHook.cpp` — DX11 publish worker、queue、cleanup、atomic frame metadata、install/uninstall rollback を追加した。
- `.agent/changes.md` — 本タスクの記録を追記した。

### Behavioral Impact
- 通常の DX11 delayed readback path では、`Present` 内での CPU payload copy と `SharedFrameWriter.WriteFrame(...)` が worker 側へ移る。
- `HT_HOOK_DISABLE_DELAYED_READBACK=1` の診断用 immediate path は従来どおり同期 publish を維持する。
- resize / detach 時は pending publish を drain してから ring/device state を破棄する。

### Risk & Mitigation
- Risk: completed slot の `Unmap` を次回 `Present` cleanup に寄せたため、slot 再利用が 1 フレーム遅れる場合がある。
- Mitigation: cleanup を `CaptureAndShareFrameLocked(...)` の先頭と reset/uninstall 経路に入れ、通常フレームでは次回 `Present` で即回収するようにした。

- Risk: install 途中失敗で publish worker が残留すると後続 attach に影響する。
- Mitigation: Hook install の各失敗経路で worker stop rollback を追加した。
...
**2026-03-16 11:23 (Asia/Taipei) — Add OCR 1% low first architecture plan**

### Summary
- OCR を維持しつつゲーム側 `1% low` を最優先するためのアーキテクチャ案を `Doc/` に追加した。

### Context / Goal
- OCR が必要な以上 CPU readback はどこかで残るが、ゲーム側 `Present` hot path を守る設計指針を明確にする必要があった。
- OCR を「全フレーム処理」ではなく「latest-only の非同期補助機能」として再定義したかった。

### Changes
- Hook / CaptureHost / OcrWorker / TranslationWorker / OverlayPublisher の責務分離案を整理した。
- latest-only queue、差分時のみ OCR、前回 overlay 維持、shared texture の位置付けを文書化した。
- `game render thread must not wait for OCR` を設計上の優先条件として明記した。

### Files Touched
- `Doc/OCR_1PercentLow_First_Architecture_Plan.md` — OCR を維持しながら 1% low を最優先するための構成、キュー戦略、実装手順、リスクを新規整理した。
- `.agent/changes.md` — 本タスクの記録を追記した。

### Behavioral Impact
- 実行時挙動の変更はまだない。
- 今後の OCR/Hook 改修で「何を同期させないか」の判断基準が明確になった。

### Risk & Mitigation
- Risk: latest-only 化により短時間しか表示されないテキストを取りこぼす可能性がある。
- Mitigation: 文書内で manual refresh、interval 調整、重要領域限定の例外運用を緩和策として残した。
