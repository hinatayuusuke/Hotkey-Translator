...
> - 本番で実行するコードを変更した時だけ追記する
> - 指示がない限り、`./.agent/changes.md`は最後の50行だけ読み込んでください
> - 追記は **1タスク=1エントリ** とし、完了時に必ず記録すること

>`Doc/`にあるファイルを読み込むためのルール
> - 必ず-Encoding UTF8パラメーターで読み込む
...
## 5. 実装案の提出フォーマット（テンプレートを固定）

* ユーザが簡易実装案だと指示している場合は簡単な文字説明で書く、それ以外は以下のテンプレートを推奨
**実装案テンプレート（推奨）**
